...
class Filter extends React.Component {
    ...
    handleCancel = () => {
        this.setState({ time: [0, 720], date: "", category: "All" })
    }
    ...
    render() {
        ...
        return (
            <>
            <Button 
                onClick={() => this.setState({ filterDisplay: (this.state.filterDisplay === 'none')? 'block' : 'none' })} 
                ...
                bgColor={'transparent'} 
                borderRadius='0px' 
                border={'1px solid rgba(73, 72, 72, 0.118)'}>
                    <HamburgerIcon />
            </Button>
            <Box as={motion.div} 
                ...
                w={['100%', '100%', '300px']} 
                h={['100%', '100%', 'auto']} 
                display={{ base: this.state.filterDisplay, sm: this.state.filterDisplay, md:'block' }} 
                zIndex={{ base: '100', sm: '100', md: '0'}} 
                position={{ base: 'fixed', sm: 'fixed', md: 'initial' }} 
                top={"50px"} right={'0'} 
                p={9} pt='4' m={2} 
                borderRadius={"10px"} 
                bgColor={"#fff"} 
            ...
            >
                ...
                <Box pt={2}>
                    ...
                    <RangeSlider defaultValue={[this.state.time[0], this.state.time[1]]} colorScheme='purple' onChangeEnd={(val) => {this.setState({ time: val })}} m={'10px 0'} max={720}>
                        <RangeSliderTrack bg={'purple.100'}>
                            <RangeSliderFilledTrack />
                        </RangeSliderTrack>
                        <RangeSliderThumb boxShadow={'2px 3px 10px rgba(73, 72, 72, 0.318)'} title={this.state.time[0]/60} index={0}/>
                        <RangeSliderThumb boxShadow={'2px 3px 10px rgba(73, 72, 72, 0.318)'} title={this.state.time[1]/60} index={1}/>
                    </RangeSlider>
                    
                    <Flex justify={'space-between'} align='center' m={'-5px'} mt='-10px' mb={2} >
                        { 
                         [0,3,6,9,12].map(text => {
                            console.log(text)
                            return <Text color='#999' fontSize='10px'>{text}h</Text>
                         })
                        }
                    </Flex>
                </Box>
                ...
                <Flex pt={2}>
                    ...
                    <Button flex={1} fontSize={10} bgColor={'purple.400'} color={"#fff"} borderRadius={"0 3px 3px 0"} border={'1px solid rgba(153, 153, 153, 0.397)'} onClick={() => {filter(this.state); this.setState({ filterDisplay: 'none' })}}>Apply</Button>
                </Flex>
            </Box>
            </>
        )
